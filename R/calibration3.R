...
#' curves (optional), omission rate, and AIC values. partial ROC is calculated
#' only for selected models.
#'
#' @usage
#' calibration3(data, test_concave = TRUE, extrapolation_factor = 0.1,
...
#' m_glm <- calibration3(data = sp_swd_glm,
#'                       test_concave = TRUE,
#'                       extrapolation_factor = 0.1,
#'                       var_limits = list("bio_7" = c(0, NA),
#'                                         "bio_12" = c(0, NA),
      ...
      if(parallel){
        results_concave <- foreach::foreach(
          ...
          .options.snow = opts) %dopar% {
            fit_eval_concave3(x = x, q_grids, data, formula_grid,
  ...
  if(n_tot == 0) {
    ...
    if(parallel){
      results <- foreach(
        ...
        .packages = c("glmnet", "enmpa"),
        .options.snow = opts ) %dopar% {
          fit_eval_models3(x, formula_grid, data,
                           omission_rate = omission_rate,
                           omrat_thr = omrat_threshold,
                           write_summary = write_summary,
                           addsamplestobackground = addsamplestobackground,
      ...
      for (x in 1:n_tot) {
        results[[x]] <-
          fit_eval_models3(x, formula_grid = formula_grid, data = data,
                           omission_rate = omission_rate,
                           omrat_thr = omrat_threshold,
                           addsamplestobackground =  addsamplestobackground,
...
fit_eval_concave3 <- function(x, q_grids, data, formula_grid, omission_rate, omrat_thr,
...
fit_eval_models3 <- function(x, formula_grid, data, omission_rate, omrat_thr,
                             write_summary, addsamplestobackground, weights,
